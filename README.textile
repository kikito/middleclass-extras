...
* *Beholder* is a small signaling/event controller mixin. Your classes can 'observe' and 'stop observing'
  events. Note that *Sender* must be required before *Beholder*, or the later will not work.
...
* The package assumes that it will be placed on a directory whose parent is included on the path.
Normally, putting it on the 'root' of your project should work. Otherwise, you will have to modify
your @package.path@ variable so includes the 'parent folder' inmediately above of middleclass-extras.
